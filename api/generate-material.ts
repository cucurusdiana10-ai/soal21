import { handleMaterialGeneration } from './_gemini';
...
export default async function handler(req: any, res: any) {
  ...
  if (req.method === 'GET') {
    return res.status(200).json({ status: 'ready', endpoint: '/api/generate-material' });
  }
  ...
  try {
    ...
    return res.status(500).json({
      error: err?.message || 'Gagal menghasilkan Bahan Ajar AI'
    });
  }
}
